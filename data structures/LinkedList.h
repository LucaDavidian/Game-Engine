...
public:
    ...
    {
    friend class LinkedList;   // corresponding instantiation of class template LinkedList is friend
    public:
...
public:
    LinkedList() : mFirst(nullptr), mLast(nullptr), mNumElements(0) {}
    ...
    T &First() { return const_cast<T&>(static_cast<const LinkedList&>(*this).First()); }
    const T&First() const { if (Empty()) throw ListEmptyException(); return mFirst->data; }
    T &Last() { return const_cast<T&>(static_cast<const LinkedList&>(*this).Last()); }
...
template <typename T>
LinkedList<T>::LinkedList(const LinkedList &other) : mFirst(nullptr), mLast(nullptr), mNumElements(0)
{
    LinkedList::ConstIterator it = other.Begin();
...
template <typename T>    
LinkedList<T>::LinkedList(LinkedList &&other) : mFirst(other.mFirst), mLast(other.mLast), mNumElements(other.mNumElements)
{
...
template <typename T>
template <typename U>
void LinkedList<T>::InsertFirst(U &&element)
{
...
template <typename T>
template <typename U>
void LinkedList<T>::InsertLast(U &&element)
{
    Node *newNode = new Node{ std::forward<U>(element), nullptr, nullptr };
...
template <typename T>
template <typename U>
typename LinkedList<T>::Iterator LinkedList<T>::Insert(const Iterator &position, U &&element)
{
    Node *newNode = new Node{ std::forward<U>(element), nullptr, nullptr };
...
template <typename T>
typename LinkedList<T>::Iterator LinkedList<T>::Remove(const Iterator &position)
{
    if (Empty())
        throw ListEmptyException();
...
template <typename T>
typename LinkedList<T>::Iterator LinkedList<T>::Find(const T &key) 
{
